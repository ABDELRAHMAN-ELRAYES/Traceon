...
void ProtocolValidator::process(const TLP &tlp) {
  std::uint64_t currentIndex = tlp.index();

  // VAL-007
  if (tlp.tc() > 7) {
    errors_.push_back({"VAL-007", ValidationType::INVALID_FIELD_VALUE,
  ...
  if (type == TlpType::MRd || type == TlpType::MWr) {
    ...
    if (lengthDw == 0) {
      errors_.push_back({"VAL-008", ValidationType::INVALID_FIELD_VALUE,
                         currentIndex, std::nullopt, "Zero-length request."});
    } else {
      // VAL-005
      if (tlp.address().has_value()) {
        std::uint64_t addr = tlp.address().value();
        if (addr % (lengthDw * 4) != 0) {
          errors_.push_back({"VAL-005", ValidationType::ADDRESS_MISALIGNMENT,
                             currentIndex, std::nullopt,
    ...
    if (type == TlpType::MRd) {
      ...
      if (it != outstanding_requests_.end()) {
        errors_.push_back({"VAL-006", ValidationType::TAG_COLLISION,
                           currentIndex, it->second.packet_index,
                           "Tag collision: tag 0x" + std::to_string(tlp.tag()) +
    ...
    if (it == outstanding_requests_.end()) {
      if (satisfied_requests_.count(key) > 0) {
        // VAL-003
        errors_.push_back({"VAL-003", ValidationType::DUPLICATE_COMPLETION,
                           currentIndex, std::nullopt,
                           "Duplicate completion."});
      } else {
        // VAL-001
        errors_.push_back({"VAL-001", ValidationType::UNEXPECTED_COMPLETION,
                           currentIndex, std::nullopt,
                           "Completion without a matching request."});
      }
    } else {
      ...
      if (type == TlpType::CplD) {
        ...
        if (receivedBytes != requestedBytes) {
          errors_.push_back({"VAL-004", ValidationType::BYTE_COUNT_MISMATCH,
                             currentIndex, it->second.packet_index,
                             "Returned byte count (" +
...
std::vector<ValidationError> ProtocolValidator::finalize() {
  for (const auto &pair : outstanding_requests_) {
    errors_.push_back({"VAL-002", ValidationType::MISSING_COMPLETION,
...
const std::vector<ValidationError> &ProtocolValidator::errors() const {
  return errors_;
}