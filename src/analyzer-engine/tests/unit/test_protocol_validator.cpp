...
TEST_F(ProtocolValidatorTest, VAL001_UnexpectedCompletion) {
  ...
  auto errs = validator.process(cpld);
  bool found = false;
  for (const auto &err : errs) {
    if (err.rule_id == "VAL-001")
      found = true;
  }
  EXPECT_TRUE(found);
}
...
TEST_F(ProtocolValidatorTest, VAL005_AddressMisalignment) {
  ...
  auto errs = validator.process(mwr);

  bool found = false;
  for (const auto &err : errs) {
    if (err.rule_id == "VAL-005")
      found = true;
  }
  EXPECT_TRUE(found);
}
...
TEST_F(ProtocolValidatorTest, VAL006_TagCollision) {
  ...
  auto errs = validator.process(mrd2);

  bool found = false;
  for (const auto &err : errs) {
    if (err.rule_id == "VAL-006")
      found = true;
  }
  EXPECT_TRUE(found);
}
...
TEST_F(ProtocolValidatorTest, VAL008_InvalidLength) {
  ...
  EXPECT_FALSE(mrd.isMalformed());
  auto errs = validator.process(mrd);

  bool found = false;
  for (const auto &err : errs) {
    if (err.rule_id == "VAL-008")
      found = true;
  }
  EXPECT_TRUE(found);
}
